...
export default function GroupAddCourseModal({ title, onClose, onAdd, alreadyAssigned = [] }: GroupAddCourseModalProps) {
  ...
  function CourseRow({ id, code, courseTitle }: { id: number; code?: string; courseTitle: string }) {
            ...
            return (
              <div key={dir.id} className="border-b border-border last:border-0">
                ...
                {isOpen && dir.courses.map((c) => (
                  <CourseRow key={c.id} id={c.id} code={c.code} courseTitle={c.title} />
                ))}
              </div>
            );
          })}
