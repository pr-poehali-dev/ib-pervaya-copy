...
function getWriteoffsForMonths(tenant: Tenant, months: number[], year: number) {
  ...
  return Object.values(combined).map((s) => ({ ...s, remaining: s.total - s.writeoff }));
}
...
function exportToExcel(tenant: Tenant, periodLabel: string, months: number[], year: number) {
  const writeoffs = getWriteoffsForMonths(tenant, months, year);
  const rows: string[][] = [];
  rows.push([`Закрытие периода: ${periodLabel}`]);
  rows.push([`Тенант: ${tenant.name}`, `ИНН: ${tenant.inn}`]);
  rows.push([]);
  rows.push(["Направление", "Лимит", "Списано за период", "Остаток", "% использования"]);
  writeoffs.forEach((s) => {
    const pct = s.total > 0 ? Math.round((s.writeoff / s.total) * 100) : 0;
    rows.push([s.label, String(s.total), String(s.writeoff), String(s.remaining), `${pct}%`]);
  });
...
interface TenantClosingReportModalProps {
  open: boolean;
  onClose: () => void;
}

export default function TenantClosingReportModal({ open, onClose }: TenantClosingReportModalProps) {
      ...
      <div className="bg-background rounded-2xl border border-border shadow-2xl w-full max-w-3xl max-h-[92vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold text-base leading-tight">Закрытие периода</h2>
              <p className="text-xs text-muted-foreground">Списание подписок · <span className="font-medium text-foreground">{periodLabel}</span></p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Button
              className="h-8 gap-1.5 rounded-xl gradient-primary text-white text-xs"
              onClick={() => exportToExcel(tenant, periodLabel, activeMonths, activeYear)}
            >
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {periodMode === "custom" && (
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <select value={customFrom.year} onChange={(e) => setCustomFrom((p) => ({ ...p, year: Number(e.target.value) }))}
                  className="h-9 px-3 rounded-xl border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30">
                  {YEARS.map((y) => <option key={y} value={y}>{y}</option>)}
                </select>
                ...
                <select value={customTo.year} onChange={(e) => setCustomTo((p) => ({ ...p, year: Number(e.target.value) }))}
                  className="h-9 px-3 rounded-xl border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/30">
                  {YEARS.map((y) => <option key={y} value={y}>{y}</option>)}
                </select>
              </div>
        ...
        <div className="grid grid-cols-3 gap-3 px-6 py-4 border-b border-border flex-shrink-0">
          <div className="bg-muted/40 rounded-xl px-4 py-3 text-center">
            <p className="text-xs text-muted-foreground mb-1">Направлений</p>
            <p className="text-2xl font-bold">{writeoffs.length}</p>
          </div>
          <div className="bg-muted/40 rounded-xl px-4 py-3 text-center">
            <p className="text-xs text-muted-foreground mb-1">Списано подписок</p>
            <p className="text-2xl font-bold text-violet-600 dark:text-violet-400">{totalWriteoff}</p>
          </div>
          <div className="bg-muted/40 rounded-xl px-4 py-3 text-center">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-muted/30 sticky top-0">
                <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground">Направление</th>
                <th className="px-4 py-3 text-right text-xs font-medium text-muted-foreground">Лимит</th>
                <th className="px-4 py-3 text-right text-xs font-medium text-muted-foreground">Списано</th>
                <th className="px-4 py-3 text-right text-xs font-medium text-muted-foreground">Остаток</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground w-40">Загрузка</th>
              </tr>
            </thead>
            <tbody>
              {writeoffs.map((s, idx) => {
              ...
              <tr className="border-t-2 border-border bg-muted/20">
                <td className="px-6 py-3 font-bold text-xs uppercase tracking-wide text-muted-foreground">Итого</td>
